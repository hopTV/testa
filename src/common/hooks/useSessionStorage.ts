...
const useSessionStorage = <T>(
  ...
  const [storedValue, setStoredValue] = useState<T>(initialValue) // Return a wrapped version of useState's setter function that ... // ... persists the new value to localStorage.
  ...
  const setValue: SetValue<T> = (value) => {
    ...
    try {
      ...
      window.sessionStorage.setItem(key, JSON.stringify(newValue)) // Save state
  ...
  useEventListener('storage', handleStorageChange) // this is a custom event, triggered in writeValueToLocalStorage // See: useLocalStorage()
